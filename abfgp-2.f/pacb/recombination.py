...
def pairwisefiltered(iterable,element):
    """
    @type  iterable: *
    @param iterable: python class instance that is iterable (list,tuple,...)

    @type  element: *
    @param element: key which is present in iterable

    @rtype:  list
    @return: `pairwise` combination filtered for specific occurrence
    """
    combis = []
    for combi in pairwise(iterable):
        if element in combi:
            combis.append(combi)
    return combis

# end of function pairwisefiltered
