...
def obtain_pssm_ic(seqs,start=None,end=None,report=False):
    ...
    for seq in seqs.values():
        if seq.upper().count("N") >= 1:
            nns+=1
            continue 
        if seq.upper().count("~") >= 1:
            nns+=1
            continue
        if seq.upper().count("X") >= 1:
            nns+=1
            continue 
        for i in range(0,patlen):
            base = seq[i].upper()
    ...
    # check if not al sequences contain N-residues!
    if nns == len(seqs): return [], nns
    ...
    for i in pattern_range:
        ...
        for base in list("ATGC-"):
            ...
            if base == '-':
                # do not store gaps as bases, but count them for lowering IC for ATGC-bases
                continue
